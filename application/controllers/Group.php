...
class Group extends CI_Controller
{
	...
	public function index($value='')
	{
		...
		$content = '<table class="table table-bordered"><thead><th>Group name</th><th>Definition</th><th>Access Request</th><th>Action</th></thead>';
		foreach ($groups as $key) {
			# code...
			$total_req = count($this->accessed_model->list_request($key->id));
			$content .= "<tr><td>$key->name</td><td>$key->definition</td><td align='center'><a href='".site_url('group/request/').$key->id."' class='btn btn-default'>".$total_req."</a></td><td><a href='".site_url('group/edit')."' class='btn btn-default'><i class='fa fa-edit'></i></a></td></tr>";
		}
		$content .= '<p>';
		$data['content'] = $content;
		$data['title'] = "Group - List all";
		$data['subtitle'] = "Group - List all";
	...
	public function create($value='')
	{
        ...
        $a = $this->aauth->create_group($this->input->post('groupname'),$this->input->post('groupdefinition'));
        //print($a);
        //print('<a href="Group')
